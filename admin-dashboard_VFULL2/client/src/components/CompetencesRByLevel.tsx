...
type Competence = { id_competencer: string; competencer: string; niveaur: number };

export default function CompetencesRByLevel({ competences }: { competences: Competence[] }) {
  const grouped = competences.reduce<Record<number, Competence[]>>((acc, comp) => {
    if (!acc[comp.niveaur]) acc[comp.niveaur] = [];
    acc[comp.niveaur].push(comp);
    return acc;
  }, {});
